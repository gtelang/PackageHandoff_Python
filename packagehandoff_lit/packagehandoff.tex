...
\footnotesize
\begin{list}{}{\setlength{\itemsep}{-\parsep}\setlength{\itemindent}{-\leftmargin}}
\item \NWtxtMacroDefBy\ \NWlink{nuweb6}{6}\NWlink{nuweb8}{, 8}\NWlink{nuweb9}{, 9}\NWlink{nuweb10}{, 10}\NWlink{nuweb14a}{, 14a}.
...
\caption{Intersection of two expanding wavelets along $\vec{H_iT}$. The figure shows an overlay of  snapshots of two times;
one at time $t$ (when the package has just been handed off to drone $i$ at $H_i$) and another at time $t + \delta t$
(when a wavelet corresponding a drone faster than drone $i$ meets it at at $M$)}
\end{figure}

The next chunk implements a function that computes the makespan of the delivery process, i.e. the time it takes
for the package to get from the source to the destination. In the chunk below, the \verb|drone_info| is list of 
...
\NWtarget{nuweb8}{} $\langle\,${\itshape Algorithms}\nobreak\ {\footnotesize {8}}$\,\rangle\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@@\\
...
This chunk implements the time on the global clock when two wavelets meet. More precisely, 
...
\begin{figure}[H]
\centering
   \includegraphics[width=10cm]{docs/get_interception_time.pdf}
\caption{Reference figure for the function \texttt{get\_interception\_time}.  Without loss of 
generality (by changing the coordinate system) we can assume the line $\vec{ST}$ to be straight.}
\end{figure}
...
Rearranging the terms, we get a quadratic equation that we can solve for in $x$ using standard 
non-linear solvers. 
...
\NWtarget{nuweb9}{} $\langle\,${\itshape Algorithms}\nobreak\ {\footnotesize {9}}$\,\rangle\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@ @\\
...
The next chunk contains the implementation of the algorithm \verb|algo_odw|. 

%{python-mode}%
\begin{flushleft} \small\label{scrap5}\raggedright\small
\NWtarget{nuweb10}{} $\langle\,${\itshape Algorithms}\nobreak\ {\footnotesize {10}}$\,\rangle\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@   @\\
...
\mbox{}\verb@    source = np.asarray(source)@\\
\mbox{}\verb@    target = np.asarray(target)@\\
\mbox{}\verb@    # unit vector pointing from source to target@\\
...
\mbox{}\verb@            np.linalg.norm((target-current_package_position))/ \@\\
...
\mbox{}\verb@@\\
\mbox{}\verb@          else:@\\
\mbox{}\verb@              package_handler_speed    = \@\\
\mbox{}\verb@                      drone_info[current_package_handler_idx][1] @\\
\mbox{}\verb@              current_package_position = \@\\
\mbox{}\verb@                      current_package_position + \@\\
...
\mbox{}\verb@          algo_pho_exact_given_order_of_drones(\@\\
...
\item \NWtxtMacroDefBy\ \NWlink{nuweb6}{6}\NWlink{nuweb8}{, 8}\NWlink{nuweb9}{, 9}\NWlink{nuweb10}{, 10}\NWlink{nuweb14a}{, 14a}.
\item \NWtxtMacroRefIn\ \NWlink{nuweb3}{3}.
\item \NWtxtIdentsDefed\nobreak\  \verb@algo_odw@\nobreak\ \NWlink{nuweb23}{23}, \verb@extract_coordinates,@\nobreak\ \NWtxtIdentsNotUsed.\item \NWtxtIdentsUsed\nobreak\  \verb@get_interception_time@\nobreak\ \NWlink{nuweb9}{9}, \verb@time_of_travel@\nobreak\ \NWlink{nuweb8}{8}.
\item{}
\end{list}
\vspace{4ex}
\end{flushleft}
...
\subsection{Algorithm: Repeated Bottleneck Matching}
\label{ssec:repbot}
...
more precise alongside the implementation for \verb|algo_repbot|. 
Here is the main outline of the implementation. 
...
\mbox{}\verb@@\\
\mbox{}\verb@def algo_repbot(drone_info, sources, targets, plot_tour_p = False):@\\
\mbox{}\verb@@\\
\mbox{}\verb@     @\hbox{$\langle\,${\itshape Sanity checks on input for \verb|algo_repbot|}\nobreak\ {\footnotesize \NWlink{nuweb14b}{14b}}$\,\rangle$}\verb@@\\
...
\mbox{}\verb@     current_package_handler  = [ None  for _ in range(numpackages) ]@\\
\mbox{}\verb@     current_package_position = sources@\\
\mbox{}\verb@     current_package_speed    = [ 0.0   for _ in range(numpackages)]@\\
\mbox{}\verb@     drone_wavelet_info       = [{'wavelet_center': posn,'radius':0.0} @\\
\mbox{}\verb@                                  for posn in drone_initposns]@\\
\mbox{}\verb@     drone_pool               = range(numdrones)@\\
\mbox{}\verb@@\\
\mbox{}\verb@     while not all(package_delivered_p):@\\
\mbox{}\verb@          @\hbox{$\langle\,${\itshape Detect event type}\nobreak\ {\footnotesize ?}$\,\rangle$}\verb@@\\
\mbox{}\verb@          @\hbox{$\langle\,${\itshape Process event}\nobreak\ {\footnotesize ?}$\,\rangle$}\verb@@\\
\mbox{}\verb@          @\hbox{$\langle\,${\itshape Update drone pool}\nobreak\ {\footnotesize ?}$\,\rangle$}\verb@@\\
\mbox{}\verb@@{\NWsep}
\end{list}
\vspace{-1.5ex}
\footnotesize
...
\NWtarget{nuweb14b}{} $\langle\,${\itshape Sanity checks on input for \verb|algo_repbot|}\nobreak\ {\footnotesize {14b}}$\,\rangle\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@   @\\
...
\NWtarget{nuweb15}{} $\langle\,${\itshape Basic setup}\nobreak\ {\footnotesize {15}}$\,\rangle\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@ @\\
...
\begin{description}
\item[\texttt{package\_delivered\_p}] is a Boolean list
...
\item[\texttt{current\_package\_handler}]  is a list of integer indices where 
     the \verb|i|\textsuperscript{th} element indicates the id of the drone currently 
     carrying the package along the straight line segment $S_iT_i$ enroute to $T_i$.
\item[\texttt{current\_package\_position}]  is a list of $(x,y)$ 
    coordinates where the \verb|i|\textsuperscript{th} element 
    indicates the coordinates of the drone with id
    \verb|current_package_handler[i]|. 
\item[\texttt{current\_package\_speed}] is a list keeping track of the current speed
     with which the package is moving. More precisely, the \verb|i|\textsuperscript{th} element is the 
     speed of the currently handling the \verb|i|\tss{th} package. Since the packages
     are initially not being handled by any of the drones, all package speeds are initialized to 0.0.
\item[\texttt{drone\_wavelet\_info}] is a list of dictionaries, 
     one dictionary per drone. Each such dictionary keeps track of 
     information about a 
     wavelet corresponding to every drone viz. the center of the wavelet about 
     which the expansion is happening (by the \verb|wavelet_center| attribute), 
     and the current radius of the wavelet (by the \verb|radius| attribute).  
\item[\texttt{drone\_pool}] is a list of ids of drones that can be 
     considered for the bottleneck matching process for the next iteration of 
     the \verb|while| loop. In each iteration the size of this list goes down 
     by \underline{exactly} one. 
\end{description}
...
\subsubsection{Step: Detecting Event Type}
\subsubsection{Step: Process Event}
\subsubsection{Step: Update Drone Pool}
...
\NWtarget{nuweb19}{} \verb@"README.md"@\nobreak\ {\footnotesize {19}}$\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@    @\\
...
\NWtarget{nuweb20}{} \verb@"src/utils_graphics.py"@\nobreak\ {\footnotesize {20}}$\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@    @\\
\mbox{}\verb@from matplotlib import rc@\\
...
\NWtarget{nuweb21}{} \verb@"src/utils_algo.py"@\nobreak\ {\footnotesize {21}}$\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@@\\
\mbox{}\verb@import numpy as np@\\
...
\mbox{}\verb@                    algo_str = raw_input(Fore.YELLOW                                             +\@\\
\mbox{}\verb@                            "Enter algorithm to be used to compute the tour:\n Options are:\n"   +\@\\
\mbox{}\verb@                            " (repbot)     Repeated bottleneck matching \n"                            +\@\\
\mbox{}\verb@                            Style.RESET_ALL)@\\
\mbox{}\verb@@\\
\mbox{}\verb@                    algo_str = algo_str.lstrip()@\\
\mbox{}\verb@                     @\\
\mbox{}\verb@                    # Incase there are patches present from the previous clustering, just clear them@\\
\mbox{}\verb@                    clearAxPolygonPatches(ax)@\\
\mbox{}\verb@                    if   algo_str == 'repbot':@\\
\mbox{}\verb@                          tour = run.get_tour( algo_repbot, plot_tour_p=True )@\\
\mbox{}\verb@                    else:@\\
\mbox{}\verb@                          print "Unknown option. No horsefly for you! ;-D "@\\
\mbox{}\verb@                          sys.exit()@\\
\mbox{}\verb@                    applyAxCorrection(ax)@\\
...
\begin{list}{}{\setlength{\itemsep}{-\parsep}\setlength{\itemindent}{-\leftmargin}}
\item \NWtxtMacroRefIn\ \NWlink{nuweb3}{3}.
\item \NWtxtIdentsUsed\nobreak\  \verb@algo_odw@\nobreak\ \NWlink{nuweb10}{10}, \verb@algo_repbot@\nobreak\ \NWlink{nuweb14a}{14a}.
\item{}
\end{list}
\vspace{4ex}
\end{flushleft}
...
\NWtarget{nuweb30}{} $\langle\,${\itshape Plotting}\nobreak\ {\footnotesize {30}}$\,\rangle\equiv$
\vspace{-1ex}
\begin{list}{}{} \item
\mbox{}\verb@@\\
